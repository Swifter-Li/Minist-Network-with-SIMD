...
/**
   @brief rectified linear layer (y_i = max(0, x_i))
...
struct Relu {
  ...
  */
  ...
  void forward_base(tensor<real,N0,N1,N2,N3>& x, int training) {
    ...
    x_ptr = &x;
    realv tempv0, tempv1;
    const realv temp0 = _mm256_setzero_ps();
    for (idx_t i0 = 0; i0 < n0; i0++) {
      for (idx_t i1 = 0; i1 < N1; i1++) {
        for (idx_t i2 = 0; i2 < N2; i2++) {
          for (idx_t i3 = 0; i3 < N3; i3 += 8) {
            tempv0 = _mm256_loadu_ps(&x(i0,i1,i2,i3));
            tempv1 = _mm256_max_ps(tempv0, temp0);
            _mm256_storeu_ps(&y(i0,i1,i2,i3), tempv1);
          }
        }
      }
    }
...
/**
   ...
   the implementation of backward of relu.
*/
