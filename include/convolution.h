...
struct Convolution2D {
  ...
  */
  ...
  void forward_base(tensor<real,maxB,IC,H,W>& x, int training) {
    ...
    y.set_n0(B);
    x_ptr = &x;
    realv temp_v0, temp_v1, temp_v2;
    realv temp_b_val;
    for (idx_t s = 0; s < B; s++) { // for each sample
        for (idx_t oc = 0; oc < OC; oc++) {
            temp_b_val = _mm256_set1_ps(b(oc));
            for (idx_t i = 0; i < H - K + 1; i++) {
                for (idx_t j = 0; j < W - K + 1; j++) {
                    temp_v2 = _mm256_setzero_ps();
                    temp_v1 = _mm256_setzero_ps();
                    temp_v0 = _mm256_setzero_ps();
                    for (idx_t ic = 0; ic < IC; ic++) {
                        realv temp_w0 = _mm256_loadu_ps(&w(oc,ic,0,0));
                        realv temp_w1 = _mm256_loadu_ps(&w(oc,ic,1,0));
                        realv temp_w2 = _mm256_loadu_ps(&w(oc,ic,2,0));
                        realv temp_x0 = _mm256_loadu_ps(&x(s,ic,i,j));
                        realv temp_x1 = _mm256_loadu_ps(&x(s,ic,i+1,j));
                        realv temp_x2 = _mm256_loadu_ps(&x(s,ic,i+2,j));
                        temp_v2 = _mm256_fmadd_ps(temp_x2, temp_w2, temp_v2);
                        temp_v1 = _mm256_fmadd_ps(temp_x1, temp_w1, temp_v1);
                        temp_v0 = _mm256_fmadd_ps(temp_x0, temp_w0, temp_v0);        
                    }
                    temp_v0 = _mm256_add_ps(temp_v0, temp_v1);
                    temp_v0 = _mm256_add_ps(temp_v0, temp_v2);
                    temp_v0 = _mm256_add_ps(temp_v0, temp_b_val);
                    _mm256_storeu_ps(&y(s,oc,i,j), temp_v0);
                }
            }
        }
    }
      ...
      for (idx_t oc = 0; oc < OC; oc++) { // for each output channel
        for (idx_t i = 0; i < H - K + 1; i++) {   // for each output pixel
          for (idx_t j = 0; j < W - K + 1; j+= L) { // for each output pixel
            ...
            realv v[9] = {U(b(oc))};
            for (idx_t ic = 0; ic < IC; ic++) { // input channel
              v[0] += w(oc,ic,0,0) * x.V(s,ic,i,j);
              v[1] += w(oc,ic,0,1) * x.V(s,ic,i,j + 1);
              v[2] += w(oc,ic,0,2) * x.V(s,ic,i,j + 2);
              v[3] += w(oc,ic,1,0) * x.V(s,ic,i + 1,j);
              v[4] += w(oc,ic,1,1) * x.V(s,ic,i + 1,j + 1);
              v[5] += w(oc,ic,1,2) * x.V(s,ic,i + 1,j + 2);
              v[6] += w(oc,ic,2,0) * x.V(s,ic,i + 2,j);
              v[7] += w(oc,ic,2,1) * x.V(s,ic,i + 2,j + 1);
              v[8] += w(oc,ic,2,2) * x.V(s,ic,i + 2,j + 2);
            }
            for (idx_t p = 0; p < 9; p++){
              y.V(s,oc,i,j) += v[p];
            }
